...
/**
 * 文件分片管理器
 * 用于处理大文件的分片上传、合并和临时文件管理Util_file
 */
export class Util_file {
    /** 内存中存储的分片信息，key为文件ID，value为分片数组 */
    private static chunk_storage = new Map<string, any[]>();

    //保存临时分片
    static save_temp_chunk(file_id: string, file_chunk_index: number, file: Express.Multer.File) {
        // 创建临时目录路径
        ...
        // 获取或初始化分片数组
        let existing_chunks = Util_file.chunk_storage.get(file_id) || [];
        // 创建分片信息对象
        const chunk_info = {
            file_chunk_index,
        ...
        // 更新内存中的分片信息
        Util_file.chunk_storage.set(file_id, existing_chunks);
     ...
     */
    static async save_merge_chunk(file_id: string, file_name: string, file_size: number, file_sha256: string) {
        // 获取所有分片信息
        const chunk_list = Util_file.getChunks(file_id);
        ...
        if (total_size !== file_size) {
            Util_file.cleanupTempFiles(file_id);
        ...
        // 清理临时文件并从内存中删除分片信息
        Util_file.cleanupTempFiles(file_id);
        Util_file.chunk_storage.delete(file_id);
     ...
     */
    static getChunks(file_id: string) {
        return Util_file.chunk_storage.get(file_id) || [];
    }
     ...
     * @param file_id 文件唯一标识符
     */
    static cleanupTempFiles(file_id: string) {
        try {
            const temp_dir = path.join(process.cwd(), 'temp_uploads', file_id);
            if (fs.existsSync(temp_dir)) {
                // 删除临时目录中的所有文件
     ...
     * @param file_id 文件唯一标识符
     */
    static cancelUpload(file_id: string) {
        Util_file.cleanupTempFiles(file_id);
        Util_file.chunk_storage.delete(file_id);
    }
}
