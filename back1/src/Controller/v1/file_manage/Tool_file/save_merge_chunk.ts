 ...
 */
export async function save_merge_chunk(file_id: string, file_name: string, file_size: number, file_sha256: string, file_path) {
    ...
    // 创建最终文件目录222配合file_path
    const upload_dir_222 = path.dirname(file_path);//file_path的父目录
    if (!fs.existsSync(upload_dir_222)) {
        fs.mkdirSync(upload_dir_222, {recursive: true});
    }
    ...
    let final_file_path = path.join(upload_dir, final_file_name)
