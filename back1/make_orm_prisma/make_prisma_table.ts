...
export function parse_class_info<T extends new (...args: any[]) => any>(target: T,): any {
    ...
    function make(property, instance) {
        let ApiProperty = Reflect.getMetadata('swagger/apiModelProperties', target.prototype, property,);
        ...
        let Column_updatedAt = Reflect.getMetadata('Column:updatedAt', target.prototype, property,);
        let Column_createdAt = Reflect.getMetadata('Column:createdAt', target.prototype, property,);
        ...
        let ele = {
            ...
            updatedAt: Column_updatedAt,
            createdAt: Column_createdAt,
...
export function make_prisma_table(columnFields) {
    ...
    for (let i = 0; i < columnFields.Fields.length; i++) {
        ...
        if (ele.updatedAt) {
            row = `${ele.field} DateTime  @updatedAt` + ` // 自动更新时间戳`
        }

        if (ele.createdAt) {
            row = `${ele.field} DateTime  @default(now())` + ` // 自动创建时间戳`
        }
